...
export async function fetchTableData() {
  try {
    const response = await axios.get('http://127.0.0.1:8848/api/admin/drivers', {
      headers: {
    ...
    {
       format[i].DriverStatus = format[i].DriverStatus ==="true"?"良好":"异常";
    }
    return format;
  } catch (error) {
    console.error(error);
...
export function useColumns() {
  ...
  onMounted(async () => {
    ...
    console.log(fetchedData.DriverStatus);
  ...
  const filterTableData = computed(() =>
    tableData.value.filter(data =>
      !search.value || String(data.FullName).toLowerCase().includes(search.value.toLowerCase())
    )
  );
